...
async def main():
    """
    Capture audio from Frame, transcribe/translate using faster-whisper, and display the result
    """
    frame = FrameMsg()
    rx_audio = None
    
    # Mode flag: True for translation, False for transcription
    ...
    try:
        ...
        data.parsers[MODE_UPDATE_MSG] = code.parse_code
        
        -- Main app function
        function app_loop()
            -- Initial display
            frame.display.text("Frame Translation App", 10, 10)
            frame.display.text("Ready - Press Enter to start", 10, 50)
            frame.display.text("Mode: Translation", 10, 90)
            frame.display.show()
            ...
            local is_translation_mode = true
                ...
                rc, err = pcall(
                    function()
                        ...
                        if items_ready > 0 then
                            ...
                            -- Handle mode update messages
                            if data.app_data[MODE_UPDATE_MSG] ~= nil then
                                is_translation_mode = data.app_data[MODE_UPDATE_MSG].value == 1
                                if is_translation_mode then
                                    frame.display.text("Mode: Translation", 10, 90)
                                else
                                    frame.display.text("Mode: Transcription", 10, 90)
                                end
                                frame.display.show()
                                data.app_data[MODE_UPDATE_MSG] = nil
                            end
                            
                            -- Handle text update messages
                            if data.app_data[TEXT_UPDATE_MSG] ~= nil and data.app_data[TEXT_UPDATE_MSG].text ~= nil then
                                -- Clear display area for text
                                frame.display.text("                                        ", 10, 170)
                                frame.display.text("                                        ", 10, 210)
                                frame.display.text("                                        ", 10, 250)
                                frame.display.text("                                        ", 10, 290)
                                frame.display.text("                                        ", 10, 330)
        ...
        await frame.upload_file_from_string(custom_lua_app, "custom_translation_app.lua")
        ...
        await frame.send_lua("require('custom_translation_app')", await_print=True)
        ...
        # Update mode display
        await frame.send_message(0x32, TxCode(value=1 if translation_mode else 0).pack())
        ...
        while True:
            ...
            print("2. Toggle mode (current: " + ("Translation" if translation_mode else "Transcription") + ")")
            print("3. Exit")
            ...
            if choice == "3":
                ...
                print(f"Mode changed to: {('Translation' if translation_mode else 'Transcription')}")
                await frame.send_message(0x32, TxCode(value=1 if translation_mode else 0).pack())
                continue
            elif choice != "1":
                ...
                while not stop_event.is_set():
                    try:
                        ...
                        if time.time() - last_t >= STEP_SEC:
                            ...
                            # Note: We don't specify language - let faster-whisper detect it
                            ...
                            if text and text != last_terminal:
                                ...
                                # Print to terminal
                                print(f"{'Translation' if translation_mode else 'Transcription'} ({detected_language}): {text}")
                                last_terminal = text
            ...
            if buf_pcm16k.size > 0:
                ...
                await frame.send_message(0x31, "Processing final result...".encode())
                ...
                language_info = f" (Detected: {detected_language}, {language_prob:.2f})"
                
                if final_text:
                    mode_text = "Translation" if translation_mode else "Transcription"
                    print(f"Final {mode_text}{language_info}: {final_text}")
                    
                    # Add language info to display text
                    display_text = f"{mode_text} ({detected_language}):\n{final_text}"
                    await frame.send_message(0x31, display_text.encode())
                else:
